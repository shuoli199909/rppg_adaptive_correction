...
class GroundTruth():
    ...
    def get_GT(self, specification, num_frame_interp, slice):
        ...
        num_frame_interp: Total number of frames after interpolation. If num_frame_interp is None, then no interpolations.
        ...
        if num_frame_interp is None:
            num_frame_interp = len(gtTime)
        gtTime = resample(x=gtTime, num=num_frame_interp)
...
class FaceDetector():
    ...
    def extract_RGB(self, img, loc_landmark):
        ...
        Returns
        -------
        sig_rgb: RGB signal of the current frame as a numpy array. Size=[3].
        """
        ...
        if (np.isnan(loc_landmark)).any() == True:
            ...
            sig_rgb = np.zeros(shape=[self.list_roi_num.shape[0], 4])
            # Loop over all ROIs.
            zeros = np.zeros(img.shape, dtype=np.uint8)
            for i_roi in range(0, self.list_roi_num.shape[0]):
                # Create the current ROI mask.
                roi_name = self.list_roi_name[i_roi]
                mask = cv2.fillPoly(zeros.copy(), [loc_landmark[self.list_roi_num[self.list_roi_name==roi_name][0].astype(int), :2].astype(int)], color=(1, 1, 1))
                # Only compute on a specific ROI.
                img_masked = np.multiply(img_RGB, mask)
                # Compute the RGB signal.
                sig_rgb[i_roi, 0:3] = 3*img_masked.sum(0).sum(0)
                # Mask size.
                sig_rgb[i_roi, 3] = mask.sum()
            sig_rgb = np.sum(sig_rgb[:, 0:3], axis=0)/np.sum(sig_rgb[:, 3])
...
def vid_to_sig(dir_vid, Params):
    ...
    while(video_crt.isOpened()):
        ...
        # Extract RGB signal.
        sig_rgb = Detector_crt.extract_RGB(img=img_frame, loc_landmark=loc_landmark)  # Size = [3].
        # ROI name.
        df_rgb.loc[num_frame, 'ROI'] = Params.list_roi_name
        # RGB channels.
        df_rgb.loc[num_frame, ['R', 'G', 'B']] = sig_rgb
        # Frame number.
        df_rgb.loc[num_frame, 'frame'] = num_frame
        # Time of the current frame.
        df_rgb.loc[num_frame, 'time'] = num_frame / Params.fps
        # Move to the next frame.
        num_frame = num_frame + 1
    # Change data format into numeric.
    df_rgb[['frame']] = df_rgb[['frame']].astype('int')
    df_rgb[['time', 'R', 'G', 'B']] = df_rgb[['time', 'R', 'G', 'B']].astype('float')
    # Dataframe reindex.
...
def sig_windowed_to_bvp(sig_rgb_win, method, Params):
    ...
    Parameters
    ----------
    sig_rgb_win: The windowed rgb signals. Size: [num_windows, rgb_channels, num_frames].
...
def rgb2bvpwin_pipe(sig_rgb, method, Params):
    """The complete pipeline of transforming raw RGB traces into BVP & HR signals.

    Parameters
    ----------
    sig_rgb: The extracted RGB signal of different ROIs. Size: [num_frames, num_ROI, rgb_channels].
    method: Selected rPPG algorithm. ['CHROM', 'LGI', 'POS', 'OMIT'].
    Params: A class containing the pre-defined parameters for the preliminary analysis.

    Returns
    -------
    sig_bvp_win: Windowed blood volume pulse (BVP) signal of different ROIs. Size=[num_windows, num_frames, num_ROI].
    time_win: Windowed time stamps. Size=[num_windows, num_frames].
    """
    ...
    # Generate windowed time stamps.
    time_win = np.zeros(shape=[len(sig_bvp_win_filtered), sig_bvp_win_filtered[0].shape[1]])
    for i_window in range(0, time_win.shape[0]):
        # Time stamp of the current window center.
        timeES_crt = timeES[i_window]
        timeES_start = timeES_crt-(0.5*(1+time_win.shape[1])-1)/Params.fps
        timeES_stop = timeES_crt+(time_win.shape[1]-0.5*(1+time_win.shape[1]))/Params.fps
        time_win[i_window, :] = np.linspace(start=timeES_start, stop=timeES_stop, num=time_win.shape[1])
    

    return sig_bvp_win_filtered, time_win
