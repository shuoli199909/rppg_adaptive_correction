...
def _calculate_fft_hr_index(ppg_signal, index_filter1, fs=60, low_pass=0.65, high_pass=4):
    ...
    index = np.argmax(mask_pxx, 0)
    index_use = index_filter1.filter(index)
    print("tyf index use before after", index, index_use)
    fft_hr = np.take(mask_ppg, index_use)[0] * 60
    return fft_hr

def _calculate_fft_hr(ppg_signal, fs=60, low_pass=0.65, high_pass=4):
    """Calculate heart rate based on PPG using Fast Fourier transform (FFT)."""
    ppg_signal = np.expand_dims(ppg_signal, 0)
    N = _next_power_of_2(ppg_signal.shape[1])
    f_ppg, pxx_ppg = scipy.signal.periodogram(ppg_signal, fs=fs, nfft=N, detrend=False)
...
def calculate_metric_per_video(predictions, labels, fs=30, diff_flag=True, use_bandpass=True, hr_method='FFT', index=None):
    ...
    if use_bandpass:
        ...
        # bandpass filter between [0.65, 4] Hz, tyf use
        [b, a] = butter(1, [0.65 / fs * 2, 4 / fs * 2], btype='bandpass')
    ...
    if hr_method == 'FFT':
        ...
        if index is not None:
            hr_pred_index = _calculate_fft_hr_index(predictions,index,fs=fs)
        print("tyf hr_label before after", hr_pred, hr_pred_index)
    elif hr_method == 'Peak':
    ...
    return hr_label, hr_pred, hr_pred_index, SNR, macc